__all__ = ["AudioSource", "Microphone"]

from scipy.io import wavfile
import pyaudio

class DeviceTypeError(Exception): pass

class AudioSource(object):

	def __init__(self):
		raise NotImplementedError("You must implememnt __init__  method.")

	def __enter__(self):
		raise NotImplementedError("You must implememnt __enter__  method.")

	def __exit__(self, exc_type, exc_value, traceback):
		raise NotImplementedError("You must implememnt __exit__  method.")
...
class Microphone(AudioSource):
	...
	def __init__(self, device_index = None, sample_rate = None, device_type = "input", chunk_size = 1024, channels = 1):
		...
		self.__bit_width = pyaudio.paInt16 # 16-bit bit width.
		self.__byte_width = pyaudio.get_sample_size(self.BIT_WIDTH) # size of each sample in bytes.
